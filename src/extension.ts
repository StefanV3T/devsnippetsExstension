...
vscode.commands.registerCommand('devsnippet.insertSnippetFromContext', async () => {
  ...
  if (!filterOption) return; // User cancelled
  ...
  vscode.commands.registerCommand('devsnippet.clearTagFilter', () => {
	...
	vscode.commands.registerCommand('devsnippet.filterByTag', (tagName: string) => {
...
vscode.commands.registerCommand('devsnippet.editSnippet', (itemOrId: any) => {
...
vscode.commands.registerCommand('devsnippet.deleteSnippet', async (itemOrId: any) => {
  ...
  const confirm = await vscode.window.showWarningMessage(
    `Are you sure you want to delete the snippet "${snippet.title}"?`,
    { modal: true },
    'Delete',
    'Cancel'
  );
...
vscode.commands.registerCommand('devsnippet.copySnippet', (itemOrId: any) => {
  // Extract snippet ID whether we get a string ID or a tree item
  const snippetId = typeof itemOrId === 'string' ? itemOrId : itemOrId?.snippetId;
